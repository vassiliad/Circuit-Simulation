...
cs *cs_load(char *matrixFilename) {
	...
	double i, j; /* use double for integers to avoid int conflicts */
	double x;
	cs *T;
	FILE *matrixFilePtr;
...
csi cs_lusol (css *S, csn *N, double *b, double *x, int n )
{
    csi ok ;
    ok = (S && N && x) ;
    if (ok)
...
csi cs_cholsol (css *S, csn *N , double *b, double *x, int n)
{
    csi ok = (S && N && x) ;
    if (ok)
    {
